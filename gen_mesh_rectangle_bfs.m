function gen_mesh_rectangle_bfs(N, M, xmin, xmax, ymin, ymax)
...
for i = 0 : M-1
  for j = 1 : N
    fprintf(file, '%5d  %5d  %5d  %5d \n', ...
	    i*(N+1)+j , i*(N+1)+j+1, (i+1)*(N+1)+j+1, (i+1)*(N+1)+j);
  end
end
fclose(file);
...
% next the nodes at the lateral side (x = xmax)
for i = 1 : M-1
    fprintf(file, '%4d \n', i*(N+1)+N+1);  %  derecha
end
% the nodes at the top (y = ymax)
for j = 1 : N+1
  fprintf(file, '%4d \n', (N+1)*M+j);
end
% next the nodes at the lateral side (x = xmin)  
for i = 1 : M-1
    fprintf(file, '%4d \n', i*(N+1)+1);    %  izquierda
end
